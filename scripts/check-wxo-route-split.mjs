...
if (count(main(wxo), /document-processing\/current\/classify-setup\.png/gi) !== 1 || !main(wxo).includes('pilot-bridge-thumbnail')) fail('wxO handoff must use exactly one authentic current Classify thumbnail without embedding the feature arc.');
...
for (const file of [
  ...
  if (file !== 'classify-setup.png') forbid(wxo, new RegExp(file.replace('.', '\\.')), `wxO umbrella page must not embed Document Processing board ${file}.`);
}
...
if ((manifest.assets ?? []).filter((asset) => asset.route === 'source-only').length !== 20) fail('Manifest must retain twenty superseded derivatives as source-only provenance.');
