      ...
      for (const theme of ['light', 'dark']) {
        ...
        const state = await cdp.evaluate(`(async()=>{
          ...
          return {
            ...
            v2BoardsLoaded:(()=>{const expected=[[2944,2112],[2952,2688],[2992,2160],[2952,2840]];const images=[...document.querySelectorAll('.wxo-v2-evidence img')];return images.length===expected.length&&images.every((image,index)=>image.complete&&image.naturalWidth===expected[index][0]&&image.naturalHeight===expected[index][1])})(),
            ...
            v2PrototypeWalkthrough:(()=>{const boundary=document.querySelector('.wxo-v2-prototype-boundary');const media=boundary?.querySelector('video');const rect=media?.getBoundingClientRect();return {published:boundary?.dataset.prototypeEvidence==='published-editorial-walkthrough',count:boundary?.querySelectorAll('video').length??0,controls:Boolean(media?.controls),autoplay:Boolean(media?.autoplay),loop:Boolean(media?.loop),playsInline:Boolean(media?.playsInline),preload:media?.preload??null,poster:media?.getAttribute('poster')??null,sources:media?.querySelectorAll('source').length??0,ready:media?.readyState??0,width:rect?.width??0,height:rect?.height??0,left:rect?.left??0,right:rect?.right??0}})(),
        ...
        if(name==='wxo') assert(
          ...
          state.v2GridColumns===1&&state.v2PrototypeWalkthrough.published&&state.v2PrototypeWalkthrough.count===1&&state.v2PrototypeWalkthrough.controls&&!state.v2PrototypeWalkthrough.autoplay&&!state.v2PrototypeWalkthrough.loop&&state.v2PrototypeWalkthrough.playsInline&&state.v2PrototypeWalkthrough.preload==='metadata'&&state.v2PrototypeWalkthrough.poster==='assets/wxo-canvas-v2/media/v2-builder-walkthrough-poster.png'&&state.v2PrototypeWalkthrough.sources===2&&state.v2PrototypeWalkthrough.ready>=1&&state.v2PrototypeWalkthrough.width>=Math.min(300,viewport.width-40)&&state.v2PrototypeWalkthrough.height>0&&state.v2PrototypeWalkthrough.left>=0&&state.v2PrototypeWalkthrough.right<=viewport.width&&state.v2AtCanvasEnd&&
