      ...
      for (const theme of ['light', 'dark']) {
        ...
        const state = await cdp.evaluate(`(async()=>{
          ...
          return {
            ...
            candidateVignette:(()=>{const image=document.querySelector('.pilot-vignettes img');const main=document.querySelector('.pilot-story');if(!image||!main)return null;const r=image.getBoundingClientRect();return {left:r.left,right:r.right,width:r.width,container:main.getBoundingClientRect().width,center:(r.left+r.right)/2}})(),
            ...
            evolutionPrimary:[...document.querySelectorAll('.pilot-evolution-primary')].map((node)=>{const r=node.getBoundingClientRect();return {left:r.left,right:r.right,center:(r.left+r.right)/2}}),
            flowComposition:(()=>{const root=document.querySelector('.pilot-flow-control-composition');if(!root)return null;const r=root.getBoundingClientRect();const boxes=[...root.querySelectorAll('.pilot-expansion-frame')].map((node)=>{const b=node.getBoundingClientRect();return {className:node.className,left:b.left,right:b.right,center:(b.left+b.right)/2}});return {left:r.left,right:r.right,columns:getComputedStyle(root).gridTemplateColumns.split(' ').length,boxes}})(),
        ...
        if(name==='wxo'){
          ...
          assert(state.candidateVignette&&state.candidateVignette.width>=state.candidateVignette.container*.98&&Math.abs(state.candidateVignette.center-viewport.width/2)<=2&&state.candidateVignette.left>=-1&&state.candidateVignette.right<=viewport.width+1,`${spec.file}: later illustration vignette must remain centered across the full evidence rail ${JSON.stringify(state.candidateVignette)}`);
          assert(state.evolutionPrimary.length===2&&state.evolutionPrimary.every(({left,right,center})=>left>=-1&&right<=viewport.width+1&&Math.abs(center-viewport.width/2)<=2),`${spec.file}: Agent and Workflow detail boards must remain centered ${JSON.stringify(state.evolutionPrimary)}`);
          ...
          if(!evidenceNarrow){const centered=state.flowComposition.boxes.filter(({className})=>/pilot-study--nodes|pilot-flow-control-primary/.test(className));assert(centered.length===2&&centered.every(({center})=>Math.abs(center-viewport.width/2)<=2),`${spec.file}: node states and Flow Controls must anchor the centered composition ${JSON.stringify(centered)}`);}
