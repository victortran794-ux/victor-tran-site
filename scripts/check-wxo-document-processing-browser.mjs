...
const pages = {
  wxo: { file: 'wxo-canvas.html', bodyClass: 'wxo-page', title: 'IBM watsonx Orchestrate', mainImages: 17, current: 'wxo-canvas.html?lock=1' },
      ...
      for (const theme of ['light', 'dark']) {
        ...
        const state = await cdp.evaluate(`(async()=>{
          ...
          const expected=${JSON.stringify(name === 'wxo'
            ? [[3168,2084],[2944,1984],[1440,665],[1440,412],[3168,1800],[3168,1800],[3168,1800],theme === 'dark' ? [1890,442] : [3780,884],[3168,2084],[3168,2114],[3168,2156],[1440,314],[1440,729],[1440,766],[1440,356]]
          ...
          return {
            ...
            candidateSystemColumns:gridColumns('.pilot-system-themes'),
        ...
        if(name==='wxo'){
          ...
          assert(state.candidateCount===15&&state.candidateLoaded&&state.candidateActivity===3&&state.candidateExpansions===3&&state.candidateDocs===0,`${spec.file}: approved fifteen-image theme-aware narrative or native dimensions failed ${JSON.stringify(state)}`);
          assert(state.candidateTriggers.length===15&&state.imageContainment.length===15,`${spec.file}: umbrella carousel evidence count failed`);
          ...
          assert(toggled.theme===theme&&toggled.overflow===0&&toggled.contained&&toggled.before.every((source,index)=>source!==toggled.changed[index]&&toggled.restored[index]===source)&&toggled.changed.every((source,index)=>source===expectedSource(expectedThemeImages[index],alternateTheme)),`${spec.file}: visible theme toggle did not switch and restore all eight theme-aware sources safely ${JSON.stringify(toggled)}`);
        ...
        if(name==='wxo'&&viewport.label==='390'&&theme==='light'){
          ...
          assert(galleryOpen.open&&galleryOpen.body&&galleryOpen.count==='01 / 15'&&galleryOpen.title==='Historical Canvas'&&galleryOpen.loaded&&galleryOpen.contained&&galleryOpen.focus,`wxo-canvas.html: mobile gallery open failed ${JSON.stringify(galleryOpen)}`);
          ...
          assert(galleryNext.count==='02 / 15'&&galleryNext.title==='Component showcase',`wxo-canvas.html: gallery next failed ${JSON.stringify(galleryNext)}`);
