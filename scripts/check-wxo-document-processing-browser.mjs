...
const pages = {
  wxo: { file: 'wxo-canvas.html', bodyClass: 'wxo-page', title: 'IBM watsonx Orchestrate', mainImages: 16, current: 'wxo-canvas.html?lock=1' },
  doc: { file: 'document-processing.html', bodyClass: 'doc-processing-page', title: 'Document Processing', mainImages: 12, current: null },
};
      ...
      for (const theme of ['light', 'dark']) {
        ...
        const state = await cdp.evaluate(`(async()=>{
          ...
          return {
            ...
            candidateSystemColumns:gridColumns('.pilot-exploration-grid--node'),
            ...
            currentFramesLoaded:[...document.querySelectorAll('.doc-current-frame img')].every((image)=>image.complete&&[[2624,1692],[1376,1048],[1584,1042]].some(([width,height])=>image.naturalWidth===width&&image.naturalHeight===height)),
        ...
        if(name==='wxo'){
          ...
          if(viewport.width<=1000){
            ...
            const [openingTitle,nodeLibraryTitle,...chapterTitles]=state.spineGeometry.sectionTitles;
            assert(near(openingTitle,expectedText)&&near(nodeLibraryTitle,expectedText)&&chapterTitles.every((left)=>near(left,state.spineGeometry.bridgeTitle)),`${spec.file}: desktop opening and stacked Node library headings must use the primary rail while later chapters align ${JSON.stringify(state.spineGeometry)}`);
          }
          assert(state.candidateCount===14&&state.candidateLoaded&&state.candidateActivity===3&&state.candidateExpansions===3&&state.candidateDocs===0,`${spec.file}: approved fourteen-image theme-aware narrative or native dimensions failed ${JSON.stringify(state)}`);
          assert(state.candidateTriggers.length===14&&state.imageContainment.length===14,`${spec.file}: umbrella carousel evidence count failed`);
          ...
          const expectedThemeImages=[
            ['home-thumbnail','protected/wxo/images/current/01-skill-studio-main.png','images/wxo-canvas/wxo-home-thumbnail-dark.png',{light:[1024,686],dark:[1079,722]}],
            ['historical-canvas','protected/wxo/assets/theme-sequences/current-workflow-light.png','protected/wxo/assets/theme-sequences/current-workflow-dark.png',{light:[3168,2084],dark:[3168,2084]}],
            ['node-key-states','protected/wxo/assets/public-candidate/15-node-key-states-light.png','protected/wxo/assets/public-candidate/15-node-key-states-dark.png',{light:[1440,665],dark:[1440,665]}],
            ['node-size-variants','protected/wxo/assets/public-candidate/16-node-size-variants-light.png','protected/wxo/assets/public-candidate/16-node-size-variants-dark.png',{light:[1440,412],dark:[1440,412]}],
            ['form-workflow','protected/wxo/assets/theme-sequences/form-workflow-light.png','protected/wxo/assets/theme-sequences/form-workflow-dark.png',{light:[3168,1800],dark:[3168,1800]}],
            ['form-configuration','protected/wxo/assets/theme-sequences/form-configuration-light.png','protected/wxo/assets/theme-sequences/form-configuration-dark.png',{light:[3168,1800],dark:[3168,1800]}],
            ['form-summary','protected/wxo/assets/theme-sequences/form-summary-light.png','protected/wxo/assets/theme-sequences/form-summary-dark.png',{light:[3168,1800],dark:[3168,1800]}],
            ['illustration-vignettes','protected/wxo/assets/public-candidate/06-illustration-vignettes.png','protected/wxo/assets/public-candidate/20-illustration-vignettes-dark.png',{light:[3780,884],dark:[1890,442]}],
            ['current-workflow','protected/wxo/assets/theme-sequences/current-workflow-light.png','protected/wxo/assets/theme-sequences/current-workflow-dark.png',{light:[3168,2084],dark:[3168,2084]}],
            ['v2-workflow','protected/wxo/assets/theme-sequences/v2-workflow-light.png','protected/wxo/assets/theme-sequences/v2-workflow-dark.png',{light:[3168,2114],dark:[3168,2114]}],
            ['v2-agent-flow','protected/wxo/assets/theme-sequences/v2-agent-flow-light.png','protected/wxo/assets/theme-sequences/v2-agent-flow-dark.png',{light:[3168,2156],dark:[3168,2156]}],
            ['flow-control-elements','protected/wxo/assets/public-candidate/17-flow-control-elements-light.png','protected/wxo/assets/public-candidate/17-flow-control-elements-dark.png',{light:[1440,314],dark:[1440,314]}],
            ['flow-control-containers','protected/wxo/assets/public-candidate/18-flow-control-containers-light.png','protected/wxo/assets/public-candidate/18-flow-control-containers-dark.png',{light:[1440,729],dark:[1440,729]}],
            ['application-example','protected/wxo/assets/public-candidate/19-application-example-light.png','protected/wxo/assets/public-candidate/19-application-example-dark.png',{light:[1440,766],dark:[1440,766]}],
            ['workflow-anchors','protected/wxo/assets/public-candidate/21-workflow-anchors-light.png','protected/wxo/assets/public-candidate/21-workflow-anchors-dark.png',{light:[1440,356],dark:[1440,356]}],
          ];
          const expectedSource=(entry,selectedTheme)=>entry[selectedTheme==='light'?1:2];
          assert(state.themeImages.length===15&&expectedThemeImages.every((entry,index)=>{const [name,light,dark,dimensions]=entry;const image=state.themeImages[index];return image?.name===name&&image.complete&&image.src===expectedSource(entry,theme)&&JSON.stringify(image.natural)===JSON.stringify(dimensions[theme])&&image.light===light&&image.dark===dark; }),`${spec.file}: theme image source or intrinsic dimensions failed at ${viewport.label} ${theme} ${JSON.stringify(state.themeImages)}`);
          assert(state.themeSequence?.tag==='OL'&&state.themeSequence.contained&&JSON.stringify(state.themeSequence.labels)===JSON.stringify(['01 · Current builder','02 · V2 workflow evolution','03 · V2 agent hierarchy'])&&state.themeSequence.screens.length===3&&state.themeSequence.screens.every((screen,index)=>screen.inside&&screen.source===expectedSource(expectedThemeImages[index+8],theme)&&JSON.stringify(screen.natural)===JSON.stringify(expectedThemeImages[index+8][3][theme])),`${spec.file}: Canvas evolution sequence failed ${JSON.stringify(state.themeSequence)}`);
          const alternateTheme=theme==='light'?'dark':'light';
          const toggled=await cdp.evaluate(`(async()=>{const images=[...document.querySelectorAll('[data-wxo-theme-image]')];const before=images.map((image)=>image.getAttribute('src'));document.querySelector('.lens-switcher [data-lens=${JSON.stringify(alternateTheme)}]').click();await new Promise((resolve)=>setTimeout(resolve,80));await Promise.all(images.map(async(image)=>{try{await image.decode()}catch{}}));const changed=images.map((image)=>image.getAttribute('src'));document.querySelector('.lens-switcher [data-lens=${JSON.stringify(theme)}]').click();await new Promise((resolve)=>setTimeout(resolve,80));await Promise.all(images.map(async(image)=>{try{await image.decode()}catch{}}));return {before,changed,restored:images.map((image)=>image.getAttribute('src')),theme:document.documentElement.dataset.theme||'light',overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth,contained:images.every((image)=>{const r=image.getBoundingClientRect(),b=(image.closest('button')||image.closest('picture')||image.parentElement)?.getBoundingClientRect();return b&&r.left>=b.left-1&&r.right<=b.right+1&&r.top>=b.top-1&&r.bottom<=b.bottom+1})}})()`);
          assert(toggled.theme===theme&&toggled.overflow===0&&toggled.contained&&toggled.before.every((source,index)=>source!==toggled.changed[index]&&toggled.restored[index]===source)&&toggled.changed.every((source,index)=>source===expectedSource(expectedThemeImages[index],alternateTheme)),`${spec.file}: visible theme toggle did not switch and restore all fifteen theme-aware sources safely ${JSON.stringify(toggled)}`);
        ...
        if(name==='doc'){
          ...
          assert(state.currentStages===4&&state.currentFrames===8&&state.currentFramesLoaded&&state.currentStoryGeometry&&state.currentStoryGeometry.left>=0&&state.currentStoryGeometry.right<=viewport.width,`${spec.file}: unaltered four-stage owner-export story failed ${JSON.stringify(state)}`);
          ...
          assert(state.currentFrameRadii.length===8&&state.currentFrameRadii.every((corners)=>corners.every((radius)=>radius>=13)),`${spec.file}: owner-export screens must round all four corners consistently ${JSON.stringify(state.currentFrameRadii)}`);
          assert(state.motionFrameRadius===null,`${spec.file}: retired neutralized motion frame returned ${JSON.stringify(state.motionFrameRadius)}`);
          if(viewport.width>=1440) assert(state.currentPairGeometry.length===2&&state.currentPairGeometry.every(({right})=>right>state.currentStoryGeometry.right+100&&right<=viewport.width),`${spec.file}: paired owner-export screens must expand across the wider evidence rail ${JSON.stringify({story:state.currentStoryGeometry,pairs:state.currentPairGeometry})}`);
          ...
          assert(state.docLoop===0&&state.endingGrids===0&&state.decisionRows===0&&state.contributionRows===0&&state.videoReady===null&&state.videoSources===0&&state.videoAutoplay===null,`${spec.file}: redundant or neutralized motion content returned ${JSON.stringify(state)}`);
        ...
        if(name==='wxo'&&viewport.label==='390'&&theme==='light'){
          ...
          assert(galleryOpen.open&&galleryOpen.body&&galleryOpen.count==='01 / 14'&&galleryOpen.title==='Historical Canvas'&&galleryOpen.loaded&&galleryOpen.contained&&galleryOpen.focus,`wxo-canvas.html: mobile gallery open failed ${JSON.stringify(galleryOpen)}`);
          ...
          assert(galleryNext.count==='02 / 14'&&galleryNext.title==='Node key states',`wxo-canvas.html: gallery next failed ${JSON.stringify(galleryNext)}`);
        ...
        if(name==='wxo'&&viewport.label==='1600'&&theme==='dark'){
          ...
          for(const [selector,fileName] of [['.pilot-canvas-opening','wxo-canvas-1600-dark-canvas-opening.png'],['.pilot-vignettes','wxo-canvas-1600-dark-vignettes.png'],['.pilot-history-canvas','wxo-canvas-1600-dark-historical-canvas.png'],['.pilot-activity-epic','wxo-canvas-1600-dark-user-activity.png'],['.pilot-side-quest-bridge','wxo-canvas-1600-dark-document-processing-bridge.png'],['.pilot-theme-sequence','wxo-canvas-1600-dark-canvas-evolution.png']]){await cdp.evaluate(`(()=>{const el=document.querySelector(${JSON.stringify(selector)});scrollTo({top:el.getBoundingClientRect().top+scrollY-84,behavior:'instant'})})()`);await delay(80);await cdp.screenshot(fileName);}
