...
expect(generator.includes('data-viewer-title="Mendenhall"'),'generator: Mendenhall poster viewer title missing');
expect(generator.includes('data-viewer-picks'),'generator: viewer-only Mendenhall picks missing');
...
expect((page.match(/data-viewer-pick\b/g)||[]).length===3,'page: Mendenhall poster must expose exactly three added viewer picks');
...
expect(!page.includes('js/graphicgallery.js'),'page: deleted standalone controller must not be requested');
...
const js=read('js/main.js');expect(js.includes('data-viewer-pick'),'shared viewer: supplemental pick discovery missing');expect(js.includes('activeItems'),'shared viewer: scoped item sequence missing');
...
console.log('MENDENHALL TYPOGRAPHY CONTRACT: PASS new_assets=3 gallery_poster=1 viewer_picks=4 rights=self-authored');
