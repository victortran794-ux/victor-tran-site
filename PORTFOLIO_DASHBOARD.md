...
In the approved local candidate, wxO is public, indexable, and included in the sitemap. Document Processing is public. `/document-processing` is a public direct route, indexable and included in the sitemap, with `manifest nav=false/homepage=false`, and linked from wxO as its feature deep dive. Document Processing is a public chapter inside wxO, not a separate Home card. Only its eight audited current owner exports are promoted; historical `/protected/wxo/` resources remain guarded. This supersedes protected-Doc instructions below for candidate source only. Commit, Preview push, merge, and production remain separate gates; this paragraph does not claim the latest local change is deployed.
...
Victor's August 7 desk review remains the content authority for the completed enhancement cycle. The bounded page, shell, gallery, accessibility, recruiter-facing alignment, and three-route Theme Continuity lanes are production-verified through PR #152; the Vercel application was subsequently submitted and closed. The Mendenhall delivery optimization is production-verified through PR #153 at `00364c03e3a522274027bf538be8c0d420d26a2e`. The clearer Light/Dark treatment is production-verified across every live shared-shell route through PR #154 at `8644d9f65f62e2ef9540b94d883d3208f038d802`, while route-specific Theme Continuity content remains scoped to Home, IBM Cloud, and About. The optional PCI orientation proof was evaluated and discarded. Recruiter-scan, IBM Cloud evidence, Pi Kapp authorship, and consolidated public-route authorship work are production-verified through PR #160 at `8cb9ed18a2dfb18e95721c90e614c852876bded4`. PR #161 closed the authorship docket. PR #162 evolved the protected wxO story from V1 foundation to V2 studies while keeping Document Processing as its subordinate protected chapter. PR #163 released the name-first Victor TRAN Homepage hero and deliberate Design DNA entry. PR #164 renamed the public UI Gallery label to Interface Studies while preserving the stable `/uigallery` route. PR #165 expanded Interface Studies with the complete Ekos desktop/mobile pair, six curated Magi boards using fictional sample data, and a refreshed Ekos homepage thumbnail at `a13d369780f599efd4b148582bec7a452fe0908c`. wxO remains password-gated, `noindex`, and omitted from the sitemap. Document Processing remains a protected chapter inside wxO rather than a separate public route. Getting In is retired from the public portfolio. Pi Kapp and Heart of the Frozen Void are public and closed.

The **authorship and website-evidence readiness** lane and its approved wxO, Homepage, and Interface Studies sequence are complete. PR #167 at `974ae9d6c2b05710052750a6d6d7bb280d8ef282` released the path-aware Site health workflow, and PR #168 retired the standalone Pi Kapp V2 runtime while preserving the stable case study and authored evidence. The portfolio is production-verified through PR #168 at `a922a27fd4c7dcd4bc4cf73ba411dcc4faf882cd`. No implementation lane is automatically active. Select the next bounded lane only from fresh source, live-site, interview, or user-priority evidence. KitBitz is tracked separately as an external illustration resource, not as a portfolio or Galactic Zoo product direction. Presentation preparation remains interview-triggered rather than part of the website queue.
...
2. Keep the released wxO, Homepage, Design DNA, Interface Studies, Site health, and Pi Kapp lanes closed unless fresh source, live-site, interview, or user-priority evidence justifies a bounded reopening.
3. Select the next bounded lane only from fresh source, live-site, interview, or user-priority evidence. Use targeted source intake only; do not repeat a broad asset inventory or manufacture work to keep the queue active.
4. Preserve the path-aware Site health workflow and layered portfolio-specific contracts. Branch protection and repository rules remain unchanged; any enforcement change requires a separate control proposal and Victor's explicit approval.
5. Preserve privacy and provenance classification. Realistic labels, metrics, names, dates, status text, or topology are not proof that a design screen contains live or private records. Verify source and provenance before classifying exposure; fail closed while provenance is unknown. Conversely, a fictional-sample-data label never overrides evidence that real client, employee, customer, or operational data is present.
6. Preserve exact-SHA post-merge GitHub Actions, Vercel deployment, live custom-domain, and browser-console verification for deployable releases.
7. Keep generic gallery overlays and A2UI parked. Retire `UI Fragments` as a separate concept. Track KitBitz separately under `C:\Users\Victor\Documents\Design\Resources\ILLUSTRATION_RESOURCES.md` as an external illustration resource, not a portfolio visual direction or Galactic Zoo roadmap.
8. Keep historical worktrees frozen and preserve the `portfolio-dashboard checkpoint before PR 95 main sync` stash as recovery material only; never apply it as current status.
...
Status: the public portfolio is production-verified through PR #168 at `a922a27fd4c7dcd4bc4cf73ba411dcc4faf882cd`. The authorship and website-evidence audit, protected wxO V1/V2 story, name-first Victor TRAN Homepage hero, Design DNA entry, expanded Interface Studies archive, path-aware Site health workflow, and Pi Kapp static evidence simplification are closed. Document Processing remains the smaller protected chapter inside wxO, and protected exports remain source-independent public stubs omitted from the public index. No implementation lane is automatically active. Every visual, narrative, route, protection, workflow-enforcement, or publication change still requires a bounded proposal, preview when applicable, verification, and explicit release approval.
...
- Pi Kapp's source-first case study, cleaned identity-pattern presentation, static V2 evidence, and source-faithful remaster are production-verified through PR #168. The standalone V2 runtime and its build/dependency surface are retired; the stable route, authored V1 sequence, expansion archive, three V2 states, final three-screen remaster, metadata, navigation, redirects, and verified claims remain.
...
- PR #162 released the protected wxO V1/V2 evolution and kept Document Processing subordinate. PR #163 released the name-first Victor TRAN Homepage hero and Design DNA entry. PR #164 released the Interface Studies public name while preserving `/uigallery`, approved artifacts, privacy boundaries, and behavior.
- PR #165 expanded Interface Studies with complete Ekos desktop/mobile frames, six curated Magi design boards using fictional sample data, deterministic asset provenance, and the refreshed homepage thumbnail.
- PR #166 refreshed the portfolio operating documentation. PR #167 released path-aware Site health scope while keeping branch protection and repository rules unchanged. PR #168 replaced the Pi Kapp standalone V2 runtime with three static historical states and aligned the three recurring triptychs without flattening their hierarchy.
...
- Interface Studies is public at the stable `/uigallery` route with complete scrollable Ekos desktop/mobile frames, four owner-approved Magi design studies using fictional sample data, the two rejected cropped studies excluded, and verified lightbox accessibility across Art, Graphic Design, and Interface Studies.
...
- `data/projects.json` records `manifest nav=false/homepage=false`. `/document-processing` is a protected direct route, omitted from public navigation, homepage cards, and the sitemap, and linked from wxO as its feature deep dive.
...
### Completed enhancement lanes and active maintenance

- Authorship and website-evidence readiness, wxO V1/V2, Homepage and Design DNA, Interface Studies, path-aware Site health, and Pi Kapp static evidence are complete through PR #168. No implementation lane is automatically active, and presentation work stays interview-triggered.

### Explicitly asset-dependent or separately release-gated

- No asset-dependent blocker remains from the completed cycle. wxO evidence, IBM Cloud supporting imagery, Pi Kapp media/page curation, Mendenhall, and Heart of the Frozen Void are complete. Pi Kapp demo simplification is maintenance reduction using existing authored evidence, not a request for new assets. Future supplied assets are fresh editorial decisions rather than carried backlog.
...
| Document Processing | production-verified protected chapter inside wxO | protected, `noindex`, hidden from public index, omitted from sitemap; linked from wxO as a subordinate feature deep dive at `/document-processing` | Current `main` plus `Portfolio\Packages\Document Processing\` | No current action | Preserve July 2026 release state, collaboration boundaries, sanitized media, and source-independent public stub |
...
| Interface Studies | production-verified through PR #165 at the stable `/uigallery` route with the PR #150 lightbox accessibility fix preserved | public | Current `main` plus approved authored Ekos and Magi sources | No current action | `UI Fragments` was the working name, not a second route; preserve complete Ekos desktop/mobile frames, the four-study owner-approved Magi curation, exclusion of the two rejected cropped studies, fictional-sample-data framing, source/provenance classification, and the accessible lightbox lifecycle |
...
- Historical note: PR #21 failed because `/document-processing` canonical/live link 404ed before the page existed live. Current state is intentional: `/document-processing` is a protected direct wxO feature-deep-dive route and remains omitted from the sitemap.
