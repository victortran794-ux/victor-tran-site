...
1. Scope Pi Kapp static-screen simplification as a maintenance and evidence-preservation lane. Preserve the stable `/pikappapp` case-study route, original V1 static sequence, final three-screen source-faithful remaster, authored expansion artifacts, metadata, navigation, and verified claims.
2. Remove the standalone runnable V2 demo only after its links, build dependencies, browser contracts, Lighthouse inventory, and generated exports are reconciled. Do not delete the current demo until the static replacement and exact redirect/404 policy are verified in a protected preview.
3. Complete the Site checks and balances audit. Keep the current deep portfolio-specific coverage, separate portable baseline checks from route-specific contracts, and propose GitHub enforcement without changing branch protection or workflows until Victor approves the control model.
4. Add privacy and provenance classification to Site checks and balances. Realistic labels, metrics, names, dates, status text, or topology are not proof that a design screen contains live or private records. Verify source and provenance before classifying exposure; fail closed while provenance is unknown. Conversely, a fictional-sample-data label never overrides evidence that real client, employee, customer, or operational data is present.
5. Treat the current CI shape as strong coverage with two governance weaknesses: the primary job is monolithic, and no classic branch protection or repository ruleset currently requires its result before merge. Preserve exact-SHA post-merge CI and Vercel production verification.
6. Keep generic gallery overlays and A2UI parked. Retire `UI Fragments` as a separate concept and keep the unlinked Pi Kapp system page inactive. Use targeted source intake only; do not repeat a broad asset inventory.
7. Track KitBitz separately under `C:\Users\Victor\Documents\Design\Resources\ILLUSTRATION_RESOURCES.md` as an external source for hand-drawn SVG/PNG illustrations and Figma kits. Do not treat it as a portfolio visual direction, a Galactic Zoo product roadmap, or a platform Victor should recreate.
...
Status: the public portfolio is production-verified through PR #165 at `a13d369780f599efd4b148582bec7a452fe0908c`. The authorship and website-evidence audit, protected wxO V1/V2 story, name-first Victor TRAN Homepage hero, Design DNA entry, and expanded Interface Studies archive are closed. Document Processing remains the smaller protected chapter inside wxO, and protected exports remain source-independent public stubs omitted from the public index. Pi Kapp static-screen simplification is the next bounded implementation candidate; Site checks and balances is the active governance audit. Every visual, narrative, route, protection, workflow-enforcement, or publication change still requires a bounded proposal, preview when applicable, verification, and explicit release approval.
...
- Pi Kapp's source-first case study, cleaned identity-pattern presentation, reproducible remaster, and contained runnable V2 are production-verified through PR #159. Victor has approved auditing removal of the maintenance-heavy standalone demo in favor of the existing authored static V1 sequence and final three-screen remaster. The stable case-study route remains; implementation is not yet authorized by this planning update.
...
- Interface Studies is public at the stable `/uigallery` route with complete scrollable Ekos desktop/mobile frames, six static Magi design studies using fictional sample data, and verified lightbox accessibility across Art, Graphic Design, and Interface Studies.
...
- Authorship and website-evidence readiness, wxO V1/V2, Homepage and Design DNA, and Interface Studies are complete through PR #165. No broad visual-enhancement queue is active. Pi Kapp static-screen simplification is the bounded maintenance candidate, Site checks and balances is the governance lane, and presentation work stays interview-triggered.
...
| Pi Kapp App | production-verified through PR #159; source-first authorship refinement closed | public | Current `main` plus `Website Items\Portfolio Packages\Pi Kapp\` | Static-screen simplification candidate; retain `/pikappapp` and remove the runnable V2 only after dependency reconciliation and approved preview | Preserve historical source versus remaster distinction, prototype-data labels, non-shipped framing, authored static evidence, and AI provenance boundaries; keep the unlinked `noindex` system page deferred |
